class Scene {
  constructor(max_depth = 1) {
    ...
    // Ray-marched cube
    this.geometries.add(new Geometry(GEOMETRIES.CUBE,
      [
        glMatrix.vec4.fromValues(0.1, 0.3, 0.5, 1)
      ]));
    this.geometries.get(4).setIdentity();
    this.geometries.get(4).rayTranslate(0, 0, 4);
    this.geometries.get(4).rayRotate(0.75 * Math.PI, 0, 0, 1);
    this.geometries.get(4).rayRotate(0.25 * Math.PI, 1, 0, 0);
  }
      ...
      for (var p_y = 0; p_y < this.buffer.height; p_y++) {
        for (var p_x = 0; p_x < this.buffer.width; p_x++) {
          ...
          for (var subpixel = 0; subpixel < tracker.aa * tracker.aa; subpixel++) {
            ...
            // accumulate color(s) from each supersample
            glMatrix.vec4.add(color, color, hit.hit_color);
          }
          // average the accumulated color(s)
